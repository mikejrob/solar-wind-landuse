...
| Military land | Can military land host solar? | ~0 new near-grid civilian supply; energy-security buildout for DoD's own load could reach ~1 GW; federal control, not soil class, is the barrier. 2024–27 pipeline is firm/microgrid (Puʻuloa 99 MW dual-fuel; only 6 MW new grid-tied PV); Trump-era policy cut climate framing, kept resilience — no Hawaii project found cancelled. | `notes/military-land-solar.md` | `data/oahu_military_land.csv` |
| DoD solar by branch | How much solar has each branch installed; how much could each host? | All ~77 MW grid-scale is on Navy land (Kupono 42, West Loch 20, PMRF 14); Army 0 grid-scale, ~27–35 MW housing rooftop; services own ~6% of the ~130–137 MW total. Potential also tilts Navy: most flat near-grid land, the only precedent-built tier, the ESQD buffers. | `notes/dod-solar-installed-by-branch.md` | `data/oahu_military_land.csv` |
| HECO–military relationship | What agreement governs military electricity? | A 10 USC 2688 utilities-privatization contract (SP0600-19-C-8322, D&O 37413): HECO OWNS the on-base distribution at 12 Army installations, billed separately, outside the general rate base. The RFP obliges HECO to facilitate third-party generation interconnection (§C.3.4) — no exclusivity. JBPHH was never privatized, which is why the IPP projects sit on Navy land. ~55 primary documents cached. | `notes/heco-military-relationship.md` | `data/raw/heco-military/` |
...
| 15–30% slope | Can grid-scale solar use 15–30% land? | 15–20% is buildable at a modeled ~$0.06–0.07/W premium (Ulupono/Fripp SWITCH); 20–30% is a speculative tail with no Hawaii cost basis. Aspect swings yield ±7/−12% at 30% — the band needs an aspect flag. 15–30% D/E splits 4,294 ac (15–20%) + 7,946 ac (20–30%). | `notes/slope-15-30-challenges.md` | `data/oahu_poa_aspect_slope.csv`, `data/oahu_slope_aspect_bins.csv` |
| Steep-slope precedent | Has anyone built solar on 20–30%+ slopes, at what cost? | Yes — Korea/Japan built on 27–47% grade; solar-site slopes have failed in monsoon rain, but causation is thin (mechanism sound, governments acted, no matched-control study; Atami was illegal fill, not solar). No project pairs a verified >20% slope with a published capex. Tropical volcanic islands (Réunion, Canaries) retreat to flats. | `notes/steep-slope-solar-precedent.md` | — |
| Available-land map | Where is all plausibly-available land, and the modeled subset? | Three-layer map (slope bands ≤15%/15–30%), all tenure categories, and a reproducible quasi-random 10%-of-B/C selection for Switch-Oahu. | `notes/available-land-map.md` | `data/oahu_bc_10pct_selection.csv`; F (available_land) |
...
| HECO procurement | How does the RFP work and where does it fail? | Staged RFP under Docket 2017-0352; median award-to-COD ~4.5–5.5 yr (Stage-1 built cohort ~5.4); cancellations from supply chain, credit, interconnection, price rigidity. | `notes/heco-solar-procurement.md` | `data/heco_procurement_timeline.csv` |
...
| Process cost channel | What does the friction cost? | Oʻahu award prices roughly doubled (Stage 1 ~$0.08–0.10/kWh → Stage 3 ~$0.21–0.23); deployment ~25–45 MW/yr vs ~200 MW/yr needed; as-of-right ≤20-ac (≈4–5 MW) parcels fall below the RFP >5 MW floor and have no PPA route. | `notes/process-cost-channel.md` | — |
